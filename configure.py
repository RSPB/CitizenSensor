...
def check_file_exist(path):
    if not os.path.isfile(path):
        raise ValidateError('{0} does not exist'.format(path))
    if not os.access(path, os.R_OK):
        raise ValidateError('{0} is not accessible'.format(path))
    return path
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', help='Path to ini config file', default='config.ini', type=check_file_exist)
    parser.add_argument('-s', '--spec', help='Path to ini config spec file', default='configspec.ini', type=check_file_exist)
